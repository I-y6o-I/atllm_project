...
async def startup_events(app: FastAPI):
    ...
    app.state.summarizer_agent = SummarizerAgent()
    logger.info("Summarizer Agent successfully loaded")

    app.state.raptor_repository = RaptorRepository(
        summarizer=app.state.summarizer_agent,
        minio_repo=MinioRepository()
    )
